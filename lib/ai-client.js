...
function extractJSON(text) {
  ...
  try { return JSON.parse(json); } catch {}
  console.error(`[extractJSON] Parse failed. JSON (first 500): ${json.slice(0, 500)}`);
  throw new Error('Réponse IA invalide. Réessaie.');
}
  ...
  return async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (status === 402) throw new Error(`Crédits insuffisants pour ${model} — ajoute des fonds sur ${endpoint.includes('openrouter') ? 'OpenRouter' : 'le fournisseur'}`);
        if (status === 404) throw new Error(`Modèle "${model}" indisponible — sélectionne un autre modèle`);
        console.error(`[ai-client] HTTP ${status} — ${endpoint} — modèle: ${model} — body: ${body.slice(0, 300)}`);
...
async function generateArticle(news, feedback = '', provider = 'groq', model = null, customPrompt = null) {
  ...
  let prompt;
  if (customPrompt) {
    prompt = `Sujet libre :\n${customPrompt}\n\nRédige un post LinkedIn professionnel, humanisé et structuré sur ce sujet en lien avec les expertises d'IMMEIT. Utilise des puces (bullet points) pour les points clés.`;
  } else {
    if (!news || !news.titre) throw new Error('Actualité source requise');
    prompt = `Actualité source :\nTitre : ${news.titre}\nSource : ${news.source}\nURL : ${news.url}\nRésumé : ${news.resume}\n\nRédige un post LinkedIn professionnel, humanisé et structuré à partir de cette actualité. Utilise des puces (bullet points) pour les points clés.`;
  }
  if (feedback) prompt += `\n\nConsignes supplémentaires : ${feedback}`;

  const modelsToTry = model ? [model] : prov.models.map(m => m.id).slice(0, 2);
