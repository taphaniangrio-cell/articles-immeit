...
- Minimum 8 contractions françaises : j'ai, c'est, on fait, y'a, t'as, j'dis, c'là, n'attendez pas, c'que, j'pense, qu'on, l'industrie, d'arrêt, d'équipement, etc.
- Minimum 3 ancrages personnels : une date précise, un chiffre client anonymisé ("un client dans l'agro", "on a vu sur un site"), une observation terrain vécue
- Alterne des phrases TRÈS courtes (3-6 mots) avec des phrases longues (25-40 mots). La variation est critique.
- Utilise le "je" ou "nous" pour créer de l'authenticité
- ZÉRO hedge words : pas de "il est important de noter", "force est de constater", "dans certains cas"
- ZÉRO ouvertures génériques : pas de "Dans un monde...", "Aujourd'hui je vais..."
- ZÉRO de "En conclusion", "Pour conclure", "En résumé"
- Liste NUMÉROTÉE (1. 2. 3. 4.) pour les points clés — JAMAIS de puces ni d'astérisques
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    if (attempt > 1 && bestResult) {
      ...
      if (improvements.length > 0) {
        prompt += `\n\n=== SCORE QUALITÉ ACTUEL : ${scoreResult.total}/10 (minimum requis: ${MIN_QUALITY_SCORE}/10) ===\nAméliore ces points spécifiquement :\n${improvements.map((f, i) => `${i + 1}. ${f}`).join('\n')}`;
