...
const { scoreArticle, getImprovementFeedback, PERSONAL_ANCHORS } = require('./quality-score');

const MIN_QUALITY_SCORE = 8;
const MAX_RETRIES = 2;
const GENERATION_DEADLINE_MS = 55_000; // 55s: leaves ~35s for images + DB within Vercel 90s maxDuration
...
function postProcessArticle(article) {
  if (!article || !article.corps) return article;

  let corps = article.corps;
  ...
  if (currentAnchors.length < 3) {
    ...
    // Insert anchor sentences at strategic positions (after 2nd, 5th, 8th sentence)
    const insertPositions = [2, 5, 8];
    let injected = 0;
    for (const pos of insertPositions) {
      if (injected >= needed) break;
      const insertIdx = Math.min(pos, sentences.length);
      const anchor = ANCHOR_SENTENCES[injected % ANCHOR_SENTENCES.length];
      sentences.splice(insertIdx, 0, anchor);
      injected++;
    }
    corps = sentences.join(' ');
    log('info', 'post_process_anchors', { added: injected, total: (corps.match(PERSONAL_ANCHORS) || []).length });
  }

  // Check word count, if too short, add context
  ...
  if (wordCount < 300) {
    ...
    const needed = Math.ceil((300 - wordCount) / 20); // ~20 words per sentence
    const sentences = corps.split(/(?<=[.!?])\s+/);
    ...
    corps = sentences.join(' ');
    log('info', 'post_process_length', { before: wordCount, after: corps.split(/\s+/).filter(Boolean).length });
  }
...
- Les hashtags vont dans le champ "hashtags", PAS dans le corps`;
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    if (Date.now() > deadline) {
      log('warn', 'quality_deadline', { attempt, bestScore });
...
${scoreResult.details.personalAnchors.count === 0 ? '\n⚠ ANCRAGES: TU EN AS 0. C\'EST REJETÉ AUTOMATIQUEMENT. Ajoute AU MINIMUM 3 phrases avec des données concrètes.' : ''}`;
    ...
    for (const m of modelsToTry) {
      if (Date.now() > deadline) {
        log('warn', 'quality_deadline_inner', { attempt, model: m, bestScore });
        break;
      }
      try {
        ...
        // Post-process: fix markdown, inject anchors if missing, pad length
        try {
          postProcessArticle(parsed);
        } catch (ppErr) {
          log('warn', 'post_process_failed', { error: ppErr.message });
        }

        // Score the generated article
        let scoreResult;
        try {
          scoreResult = scoreArticle(parsed.corps || '');
        } catch (scErr) {
          log('warn', 'scoring_failed', { error: scErr.message });
          scoreResult = { total: 0, details: {} };
        }
  ...
  throw lastError || new Error(`Echec apres ${MAX_RETRIES} tentatives. Dernier modele essaye: ${modelsToTry[0] || 'aucun'}`);
}
