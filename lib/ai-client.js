...
async function generateArticle(news, feedback = '', provider = 'mistral', model = null, customPrompt = null, options = {}) {
  ...
  let prompt;
  if (options.regenerate && options.existing) {
    const ex = options.existing;
    prompt = `Améliore cet article LinkedIn existant. Garde le MÊME sujet, la MÊME structure, les MÊMES chiffres et anecdotes. Améliore uniquement : l'accroche, le ton (plus humain, moins IA), la fluidité, et la conformité aux règles LinkedIn.
...
Redige la VERSION AMELIOREE de cet article. Tu dois garder le même sujet et les mêmes idées principales. Change juste la forme pour que ça ressemble à l'écriture d'un humain expert.`;
