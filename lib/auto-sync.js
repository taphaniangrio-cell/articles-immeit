...
const GITHUB_PUBLISH_INTERVAL_MS = 60 * 60 * 1000; // 1h max

async function tryGithubPublish(data) {
  const now = Date.now();
  if (now - _lastGithubPublish < GITHUB_PUBLISH_INTERVAL_MS) {
    log('info', 'auto_sync_github_publish_skipped_throttled', { lastPublishAgo: Math.round((now - _lastGithubPublish) / 1000) + 's' });
    return;
  }
  try {
    const { publishCache } = require('./github-cache');
    await publishCache(data);
    _lastGithubPublish = now;
...
async function performSync(opts) {
  ...
  if (fileCached) {
    ...
    if (fileAge > MAX_FILE_CACHE_AGE) {
      log('info', 'auto_sync_file_cache_stale', { syncedAt: fileCached.syncedAt, ageMin: Math.round(fileAge / 60000) });
    } else {
