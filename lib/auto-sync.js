...
let _deviceCodeStarted = false;

async function getGraphToken() {
  if (_msalToken.token && _msalToken.expiresAt > Date.now() + 60000) return _msalToken.token;

  // If already waiting for device code, try to get token (silent if user authed)
  if (_deviceCodeStarted) {
    var result = await raceTimeout(_deviceCodePromise, 3000);
...
async function tryInteractive() {
  if (!_deviceCodeStarted && !_msalToken.token) return null;
  try {
...
function startBackgroundAuth() {
  if (_deviceCodeStarted || isAppOnlyConfigured()) return;
...
async function sync() {
  ...
  // 4) Cache fichier local — dernier recours
