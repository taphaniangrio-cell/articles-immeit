...
function tokenCacheFile() { return path.join(getCacheDir(), 'msal-token.json'); }

function saveTokenToFile() {
  try {
    if (_msalToken.token && _msalToken.expiresAt > Date.now()) {
      safeWriteFile(tokenCacheFile(), { token: _msalToken.token, expiresAt: _msalToken.expiresAt });
    }
  } catch {}
}
...
function loadTokenFromFile() {
  try {
    var raw = safeReadFile(tokenCacheFile());
    if (raw) {
      var data = JSON.parse(raw);
...
async function getGraphToken() {
  ...
  // Restaurer depuis le fichier si le process a redémarré
  var fileToken = loadTokenFromFile();
  if (fileToken) {
    _msalToken = fileToken;
    log('info', 'auto_sync_token_restored', { expiresAt: new Date(_msalToken.expiresAt).toISOString() });
