...
async function performSync(opts) {
  ...
  try {
    ...
    // Si l'erreur contient des indices d'auth, déclencher l'alerte reconnexion
    const msg = (e && e.message) || '';
    if (msg.includes('invalid_grant') || msg.includes('expired') || msg.includes('TokenExpired') || msg.includes('AADSTS')) {
      maybeAlertReconnectNeeded('Erreur d\'authentification: ' + msg).catch(() => {});
    }
  }
  ...
  } catch (e) { log('warn', 'auto_sync_github_cache_failed', { error: e.message }); }
