...
const { useIdentityPlugin, InteractiveBrowserCredential } = require('@azure/identity');
...
async function ensureToken() {
  ...
  tokenPromise = (async () => {
    try {
      ...
      log('info', 'auto_sync_token_acquired', { expires: resp.expiresOn });
      return resp.token;
    } catch (err) {
      tokenPromise = null;
      throw err;
...
async function fetchSharePointData() {
  ...
  try {
    ...
    const items = rows.slice(1).map((row, idx) => {
      ...
      return obj;
    }).filter(item => Object.keys(item).some(k => k !== '_row' && item[k] && item[k].trim().length > 0));

    const cacheData = { headers, items, syncedAt: new Date().toISOString(), source: 'auto_sync' };

    saveCache(cacheData);
    tryDbSave(cacheData);

    log('info', 'auto_sync_complete', { items: items.length });
    return cacheData;
  } catch (err) {
...
module.exports = { sync, startPeriodicSync, stopPeriodicSync, loadCache, fetchSharePointData, clearToken };
