...
async function maybeAlertReconnectNeeded(reason) {
  ...
  try {
    ...
    var last = r.rows[0] && r.rows[0].cache_data && r.rows[0].cache_data.sentAt;
...
async function performSync(opts) {
  ...
  if (dbCached) {
    ...
    return {
      success: true,
      count: dbCached.items.length,
      rawCount: dbCached._rawCount,
      syncedAt: dbCached.syncedAt,
      source: 'db_cache',
      message: dbCached.items.length + ' demandes (cache base de données — SharePoint indisponible)',
    };
  }
  ...
  if (fileCached) {
    return {
      success: true,
      count: fileCached.items.length,
      syncedAt: fileCached.syncedAt,
      source: 'file_cache',
      message: fileCached.items.length + ' demandes (cache local — SharePoint indisponible)',
    };
  }
...
module.exports = {
  performSync,
  sync: performSync, // alias rétro-compatible (ancien nom utilisé par du code existant)
  initialSync,
  startContinuousSync,
  stopContinuousSync,
  loadCache,
