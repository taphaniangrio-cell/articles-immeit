...
function msalCacheFile() { return path.join(getCacheDir(), 'msal-cache.json'); }
    ...
    if (sp.isConfigured()) {
      ...
      if (result && result.connected && result.items?.length > 0) {
        ...
        try {
          ...
          const report = diffDetector.buildDiffReport(cacheData, 'Client Credentials');
          if (report) emailAlert.sendAlert(report).catch(e => log('warn', 'auto_sync_cc_alert_failed', { error: e?.message }));
        } catch {}
