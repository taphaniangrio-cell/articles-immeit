...
const SYNC_DELAY = 300_000; // 5 minutes au lieu de 2s (evite rate-limiting Graph API)
let syncTimer = null;
...
async function ensureToken() {
  ...
  try {
    const resp = await withTimeout(cred.getToken('https://graph.microsoft.com/.default'), 15000);
...
async function syncLoop() {
  ...
  isSyncing = true;
  try {
    // 1) Try client_credentials (app-only, no popup, works headless)
    if (await tryClientCredentials()) return;

    // 2) Try InteractiveBrowserCredential (local dev with session)
    try {
      const token = await ensureToken();
      if (await quickCheck(token)) return;
      await fetchAndProcess(token);
    } catch {
      // 3) Silent fallback — use cache if available, no warnings
    }
  } catch {
    // total silence — ne pas polluer les logs
  } finally {
    isSyncing = false;
  }
}
...
async function initialSync() {
  // 1) Client credentials first (no popup)
  if (await tryClientCredentials()) {
    const cached = loadCache();
    return cached || null;
  }

  // 2) Interactive (local dev)
