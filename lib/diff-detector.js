...
async function saveCurrentState(data) {
  const state = { items: data.items, headers: data.headers, syncedAt: data.syncedAt, source: data.source || 'unknown', lastModifiedDateTime: data.lastModifiedDateTime || '' };
  if (process.env.DATABASE_URL) {
    try {
      await query(
...
async function buildDiffReport(newData, lastModifiedBy) {
  ...
  const fileTimestampChanged = lastModifiedDateTime && prev.lastModifiedDateTime && lastModifiedDateTime !== prev.lastModifiedDateTime;
