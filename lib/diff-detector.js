...
async function saveCurrentState(data) {
  const state = { items: data.items, headers: data.headers, syncedAt: data.syncedAt, source: data.source || 'unknown' };
  if (process.env.DATABASE_URL) {
    try {
      await query(
...
async function buildDiffReport(newData, lastModifiedBy) {
  ...
  if (prev.source && newData.source && prev.source !== newData.source) {
    ...
    if (prevIsCache && newIsLive) {
      log('info', 'diff_source_switch_skipped', { prevSource: prev.source, newSource: newData.source });
      await saveCurrentState(newData);
      return null;
    }
  }
