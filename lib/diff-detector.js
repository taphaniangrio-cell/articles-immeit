...
function findChanges(oldItems, newItems, headers) {
  ...
  const headerLower = headers.map(h => normalizeKey(h));
  ...
  for (const [id, newItem] of newMap) {
    ...
    const fieldChanges = [];
    const FIELDS_TO_IGNORE = new Set(['_row', 'nbe_gerico_apex', 'fichier_modifi']);
    const fieldsToCompare = headerLower.filter(h => !FIELDS_TO_IGNORE.has(h));
...
async function buildDiffReport(newData, lastModifiedBy) {
  ...
  if (changes.length === 0) {
    log('info', 'diff_no_changes', { version: DIFF_VERSION, fileTimestampChanged });
    await saveCurrentState(newData);
    return null;
  }
