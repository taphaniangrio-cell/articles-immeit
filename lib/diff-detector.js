...
async function loadPreviousState() {
  if (!process.env.DATABASE_URL) {
    var fromFile = loadPreviousStateFile();
    if (fromFile) sanitizeItems(fromFile.items);
    return fromFile;
  }
  try {
    var result = await query('SELECT cache_data FROM dashboard_cache WHERE cache_key = $1', [DIFF_DB_KEY]);
    if (result && result.rows && result.rows.length > 0) {
      var data = result.rows[0].cache_data;
