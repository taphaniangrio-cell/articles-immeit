...
function buildHtml(report) {
  const changes = [];
  if (report.added > 0) {
    changes.push(`${report.added} nouvelle${report.added > 1 ? 's' : ''} demande${report.added > 1 ? 's' : ''}`);
  }
  if (report.removed > 0) {
    changes.push(`${report.removed} demande${report.removed > 1 ? 's' : ''} supprimée${report.removed > 1 ? 's' : ''}`);
  }
  if (report.modified > 0) {
    changes.push(`${report.modified} demande${report.modified > 1 ? 's' : ''} modifiée${report.modified > 1 ? 's' : ''}`);
  }
...
<h1 style="margin:0;color:#fff;font-size:18px;font-weight:600">🔔 IMMEIT — Suivi modifications</h1>
<p style="margin:4px 0 0;color:#94a3b8;font-size:13px">Fichier de suivi des demandes — ${escapeHtml(fmtTime(report.detectedAt))}</p>
</td></tr>
<tr><td style="padding:20px 24px">
<p style="margin:0 0 16px;color:#334155;font-size:14px;line-height:1.6">
  <strong>${changes.join(', ')}</strong> détectée${changes.length > 1 ? 's' : ''} dans le fichier de suivi.
</p>
...
<p style="margin:0;color:#94a3b8;font-size:11px">Cet email est envoyé automatiquement par le serveur IMMEIT à chaque détection de modification dans le fichier de suivi SharePoint.</p>
</td></tr>
</table>
</td></tr></table>
</body></html>`;
...
async function sendAlert(report) {
  if (!isConfigured()) {
    log('warn', 'alert_email_not_configured', { smtp: !!process.env.SMTP_HOST, user: !!process.env.SMTP_USER });
    return false;
  }

  const toEmail = process.env.ALERT_EMAIL_TO;
  if (!toEmail) {
    log('warn', 'alert_email_no_recipient');
    return false;
  }

  const subject = `🔔 IMMEIT — ${report.added > 0 ? report.added + ' nouvelle(s) demande(s)' : ''}${report.added > 0 && report.modified > 0 ? ', ' : ''}${report.modified > 0 ? report.modified + ' modification(s)' : ''}${(report.added > 0 || report.modified > 0) && report.removed > 0 ? ', ' : ''}${report.removed > 0 ? report.removed + ' suppression(s)' : ''} détectée(s)`;
  ...
  try {
    ...
    log('info', 'alert_email_sent', { to: toEmail, messageId: info.messageId, added: report.added, modified: report.modified, removed: report.removed });
    return true;
  } catch (err) {
    log('error', 'alert_email_failed', { error: err.message, code: err.code });
    return false;
  }
}

module.exports = { isConfigured, sendAlert, buildHtml };
