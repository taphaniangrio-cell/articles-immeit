...
async function sendAlert(report) {
  ...
  if (process.env.SMTP_USER && process.env.SMTP_PASS) {
    try {
      ...
      lastError = err;
      log('error', 'alert_smtp_failed', { error: err.message, code: err.code, to: process.env.ALERT_EMAIL_TO });
    }
  } else {
    log('warn', 'alert_smtp_not_configured', { missing: !process.env.SMTP_USER ? 'SMTP_USER' : 'SMTP_PASS' });
  }

  try {
    const url = await sendEthereal(subject, htmlBody);
    log('error', 'alert_ethereal_fallback_used', {
      to: process.env.ALERT_EMAIL_TO,
      previewUrl: url,
      smtpError: lastError ? lastError.message : 'SMTP_USER/PASS not configured',
      message: 'ALERT: Email sent to Ethereal (test server) - NOT delivered to real recipient!',
    });
    return false;
  } catch (err2) {
...
module.exports = { sendAlert, buildProHtml, sendReconnectAlert };
