...
async function _refreshWithRawRefreshToken(refreshToken) {
  ...
  try {
    ...
    if (data.access_token) {
      ...
      // Also save to DB (Vercel, GitHub Actions)
      try {
        const db = require('./db');
        await db.query(
          ...
          ['msal_token_cache', JSON.stringify({ blob: JSON.stringify(entry) })]
        );
      } catch { /* ignore DB save error */ }
...
async function getDelegatedTokenSilent() {
  ...
  try {
    ...
    log('warn', 'graph_auth_get_accounts_failed', { error: e && e.message });
  }
