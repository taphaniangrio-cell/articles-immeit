...
async function findImagesForArticle({ titre_interne, hashtags, corps, image_keywords }) {
  const keywords = image_keywords && image_keywords.length > 0
    ? [...image_keywords, ...FALLBACK_QUERIES].slice(0, 6)
    : extractKeywords(titre_interne, hashtags, corps);
  const seen = new Set();
  const all = [];
  ...
  for (const kw of keywords) {
    ...
    for (const p of photos) {
      if (!seen.has(p.url) && all.length < 8) {
        seen.add(p.url);
        all.push(p);
      }
    }
