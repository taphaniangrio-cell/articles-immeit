    ...
    const req = https.request(opts, (res) => {
      ...
      res.on('end', () => {
        ...
        try {
          ...
          reject(new Error(`Parse failed: ${raw.slice(0, 200)}`));
        }
      });
    });
    req.on('error', reject);
...
function computeStats(items, headers) {
  ...
  // Variantes corrompues dans SharePoint → forme canonique (clé normalisée)
  const typeAliases = {
  ...
  items.forEach(item => {
    ...
    const typeKey = typeAliases[normalize(rawType)] || type;
    ...
    if (normalize(rawStatus) === 'terminee' || normalize(rawStatus) === 'termine') {
      completedCount++;
    }
