...
function filterDataRows(rows, headers) {
  ...
  const confCol = headers.find(h => /conformit/i.test(h));
  const confKey = confCol ? confCol.toLowerCase().replace(/[\s\/]+/g, '_').replace(/[^a-z0-9_]/g, '') : null;

  return rows.filter(row => {
    if (uniqueKeyCols.length > 0) {
      if (!uniqueKeyCols.some(k => isRealValue(row[k]))) return false;
    } else {
      let filled = 0;
    ...
    if (confKey && row.hasOwnProperty(confKey) && row[confKey] !== undefined) {
      const cv = String(row[confKey] || '').trim();
...
async function findDriveItem(token, siteId) {
  ...
  try {
    ...
    try {
      const data = await httpsRequest(
        `https://graph.microsoft.com/v1.0/sites/${siteId}/drive/root/search(q='${querystring.escape(process.env.SHAREPOINT_SHEET_NAME || 'Suivi Demandes 2026')}')`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      throw new Error('Fichier "' + (process.env.SHAREPOINT_SHEET_NAME || 'Suivi Demandes 2026') + '" introuvable dans SharePoint');
    } catch (err) {
      throw err;
    }
  }
