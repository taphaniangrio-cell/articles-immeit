...
const { canPublish, markPublished } = require('./github-throttle');
...
function saveCache(data) {
  ...
  if (safeWriteFile(f, data)) {
    ...
    if (canPublish()) {
      try { require('./github-cache').publishCache(data).then(() => { markPublished(); }).catch(() => {}); } catch {}
    }
  } else {
...
async function fetchLiveClientCredentials() {
  ...
  if (result && result.connected && result.items?.length > 0) {
    return {
      ...
      lastModifiedBy: result.lastModifiedBy || 'Inconnu',
...
async function executeSync({ source = 'manual', skipLock = false } = {}) {
  ...
  try {
    ...
    if (data) {
      ...
      modifiedBy = data.lastModifiedBy || 'Inconnu';
      dataSource = 'live';
      log('info', 'sync_engine_live_fetched', {
        syncId,
        source: 'client_credentials',
    ...
    if (!data) {
      ...
      if (data) {
        log('info', 'sync_engine_using_db_cache', { syncId, items: data.items.length, syncedAt: data.syncedAt });
        dataSource = 'db_cache';
      }
    }
    ...
    if (!data) {
      data = loadCache();
      if (data) {
        log('info', 'sync_engine_using_file_cache', { syncId, items: data.items.length, syncedAt: data.syncedAt });
    ...
    if (!data) {
      try {
        ...
        if (data) {
          log('info', 'sync_engine_using_github_cache', { syncId, items: data.items.length, syncedAt: data.syncedAt });
    ...
    if (!data || !data.items || data.items.length === 0) {
      ...
      log('error', 'sync_engine_no_data', { syncId, duration: Date.now() - startTime });
      return { success: false, reason: 'no_data', items: 0, duration: Date.now() - startTime };
    }
    ...
    if (data.headers && data.items && data.items.length > 0) {
      ...
      if (filtered.length !== data.items.length) {
        log('info', 'sync_engine_filtered', {
          syncId,
          before: data.items.length,
          after: filtered.length,
          removed: data.items.length - filtered.length,
          source: dataSource
        });
        data.items = filtered;
      }
    ...
    // Diff + alert (email sent on every change detected)
    try {
      const alertManager = require('./alert-manager');
      await alertManager.processDiff(data);
    } catch (e) {
      log('warn', 'sync_diff_alert_failed', { syncId, error: e.message });
    }
